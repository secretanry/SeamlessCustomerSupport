...
def get_similar_question_index(user_question, all_questions):
...
most_similar_question_index, similarity = get_similar_question_index(question, all_questions)

if similarity > 0.5:
    volunteer_answer = all_answers[most_similar_question_index]
    ref.child(id).push({
        'A question': question,
...
async def receive_question(question: Question):
    ...
    if all_questions_data is not None:
        ...
        most_similar_question_index, similarity = get_similar_question_index(user_question, all_questions)

        if similarity > 0.7:  # Если косинусное сходство больше 0.7
            volunteer_answer = all_answers[most_similar_question_index]
