...
def bot_with_volunteer(message):  # переписка бота с волонтёром
    answer_1 = "Answer from volunteer" #ответ волонтёра  # проверяет весь ли это ответ
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    button_1 = types.KeyboardButton("Yes")
    button_2 = types.KeyboardButton("No")
    markup.add(button_1, button_2)
    bot.send_message(message.chat.id, text="Is it all answer? (Yes or No)".format(message.from_user), reply_markup=markup)

    while message.text != "Yes": #пока ответ не весь, добавляет в ответ и снова спрашивает
        answer_more = "More details"
